...
void init_screen(){
    ...
    tft.setTextSize(4);
    tft.drawString("GeekMagic S3", 10,100);
    delay(1000);
}
...
void tDisplay_MinerScreen(unsigned long mElapsed)
{
  ...
  Serial.println("btc price:");

  render.drawString(data.currentHashRate.c_str(), 40, 150, TFT_GOLD, TFT_BLACK);
  // Total hashes
...
void tDisplay_ClockScreen(unsigned long mElapsed)
{
  ...
  background.setTextColor(TFT_GOLD);
...
void tDisplay_GlobalHashScreen(unsigned long mElapsed)
{
  ...
  render.rdrawString(data.globalHashRate.c_str(), 205, 115, TFT_CYAN);
